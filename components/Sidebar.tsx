...
import { LayoutDashboard, Globe, Users, Scale, Network, Terminal, ShieldCheck, Command, CircleDashed, CheckCircle2, AlertCircle, Activity, PackageSearch, ChevronDown, ChevronRight, Flame, MessageSquare, PlusCircle, Sparkles, ExternalLink, TrendingUp } from 'lucide-react';
import { AgentType, MCPToolStatus, ProductCatalog } from '../types';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeAgent, onSelectAgent, agentStatuses, catalogData }) => {
  // 控制目录的展开/折叠状态
  ...
  const getStatusIcon = (status: MCPToolStatus) => {
    switch (status) {
      case MCPToolStatus.RUNNING:
        return <CircleDashed size={14} className="text-nexus-accent animate-spin" />;
      case MCPToolStatus.SUCCESS:
        return <CheckCircle2 size={14} className="text-nexus-success" />;
      case MCPToolStatus.ERROR:
        return <AlertCircle size={14} className="text-red-500" />;
      default:
        return <div className="w-2 h-2 rounded-full bg-gray-700" />; // Idle
    }
  };
  ...
  return (
    <div className="w-72 bg-nexus-900 border-r border-nexus-800 flex flex-col h-screen sticky top-0 shrink-0 overflow-y-auto custom-scrollbar">
      ...
      <div className="py-4 space-y-6">
        ...
        {/* 1. 智能体矩阵 */}
        <div>
          <div className="px-6 mb-2 text-sm font-bold text-gray-500 uppercase tracking-wider flex justify-between items-center">
            <span>智能体矩阵</span>
          ...
          <div className="space-y-0.5">
            {agents.map((agent) => (
              <button
                ...
                className={`w-full px-6 py-3 flex items-center gap-3 transition-all border-l-2 relative group ${activeAgent === agent.id
                  ? 'bg-nexus-800/50 border-nexus-accent text-white'
                  : 'border-transparent text-gray-400 hover:text-gray-200 hover:bg-nexus-800/30'
                  }`}
              >
        ...
        <div>
          ...
          <div className="px-6 space-y-2">
            <div className="text-xs text-gray-400">
              <p>实时市场调研与竞品分析</p>
              <div className="mt-2 p-3 bg-nexus-800/30 border border-nexus-700/50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-gray-300">调研任务</span>
                  <span className="text-xs text-nexus-accent">进行中</span>
                </div>
                <div className="text-[10px] text-gray-500">
                  平台覆盖: Amazon, TikTok, Alibaba, AliExpress
                </div>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-1 px-3">
            ...
            <div className="overflow-hidden rounded-lg bg-nexus-800/20 border border-nexus-800/50">
              <button
                onClick={() => toggleSection('hotProducts')}
                className="w-full px-3 py-2 flex items-center gap-2 text-xs text-gray-300 hover:bg-nexus-800/40 transition-colors"
              >
                ...
                <div className="px-3 pb-2 space-y-1 animate-in slide-in-from-top-1 duration-200">
                  {catalogData.hotProducts.map(item => (
                    <a
                      ...
                      className="flex flex-col p-2 rounded hover:bg-nexus-800/50 cursor-pointer group transition-colors text-decoration-none"
            ...
            <div className="overflow-hidden rounded-lg bg-nexus-800/20 border border-nexus-800/50">
              <button
                onClick={() => toggleSection('clientInquiries')}
                className="w-full px-3 py-2 flex items-center gap-2 text-xs text-gray-300 hover:bg-nexus-800/40 transition-colors"
              >
