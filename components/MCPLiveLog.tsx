...
import { Terminal, CheckCircle2, CircleDashed, AlertCircle, ArrowRight, ArrowLeft, MessageSquareCode, Database } from 'lucide-react';
      ...
      return (
        <div className={`flex flex-col w-full p-2 rounded border my-1 transition-all ${
          ...
          <div className={`text-[10px] font-mono pl-1 border-l-2 ${
            isRequest ? 'border-blue-700 text-blue-100' : 'border-emerald-700 text-emerald-100'
          }`}>
            {hasJSON ? (
              <pre className="overflow-x-auto p-1 bg-black/20 rounded text-[9px] leading-relaxed">
                {formatJSON(content)}
              </pre>
            ) : (
              <span>{content}</span>
    ...
    return (
      <div className="flex items-start gap-2 max-w-full">
        ...
        <div className="flex-1 min-w-0">
          <span className={`font-bold mr-2 text-nexus-accent`}>
            [{log.toolName}]
          </span>
  ...
  return (
    <div className="bg-nexus-900 rounded-lg border border-nexus-700 overflow-hidden flex flex-col h-full shadow-inner">
      ...
      <div className="flex-1 overflow-y-auto p-3 font-mono text-xs space-y-2 custom-scrollbar">
        {logs.length === 0 && (
          <div className="text-gray-600 italic text-center mt-8 flex flex-col items-center gap-3">
             <Database size={20} className="opacity-20"/>
             <span className="opacity-50">Awaiting A2A Handshake...</span>
          </div>
        )}
        {logs.map((log) => (
